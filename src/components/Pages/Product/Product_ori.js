...
const Product = () => {
  ...
  const editClicked = (
    ...
    groups.forEach(gggg => {
      // console.log(grppp);
      if(gggg.group_id === grppp) setGrpNm(gggg.group_name);
    });
    categories.forEach(cccc => {
      if(cccc.category_id === cattt) setCatNm(cccc.category_name);
    });
    hsns.forEach(hhhh => {
      if(hhhh.hsn_id === hsnnn) setHsnNm(hhhh.hsn_user);
    });
    units.forEach(uuuu => {
      if(uuuu.unit_id === unttt) setUntNm(uuuu.unit_name);
    });
    setGroupID(grppp);
    setCatID(cattt);
    setHsnID(hsnnn);
